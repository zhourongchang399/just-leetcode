...
// 
//
// Related Topics 贪心 数组 排序 👍 1090 👎 0


//leetcode submit region begin(Prohibit modification and deletion)
/**
 * @author: Zc
 * @description: 先根据 xStart 进行降序排序，再对相同 xStart 按照 xEnd 进行降序排序，遍历该数组，当区间重叠则更新当前最佳射击范围，直到不重叠为止，则 res++ 并更新最佳射击区间为当前气球直径区间，直到数组遍历完成。
 * @date: 2025/5/11 21:45
 * @param null
 * @return 
 */
class Solution {
    public int findMinArrowShots(int[][] points) {
        List<int[]> collect = Arrays.stream(points).sorted((a, b) -> {
        ...
        for (int[] point : collect) {
            ...
            if (current[1] >= point[0]) {
                current = new int[]{point[0], Math.min(point[1], current[1])};
            } else {
                current = point;
                res++;
            }
